...
describe("XSD Generator - Occurrence Handling", () => {
  ...
  const occurrencesXsdPath = path.join(
    testResourcesDir,
    "compositor-occurrences.xsd"
  );
  ...
  describe("maxOccurs on compositors", () => {
    test("elements in compositors with maxOccurs > 1 are arrays", () => {
      withTmpDir((tmpDir) => {
        ...
        const content = readFileSync(
          path.join(tmpDir, "groupInUnboundedSequence.ts"),
          "utf8"
        );
    ...
    test("nested compositors with different maxOccurs values", () => {
      withTmpDir((tmpDir) => {
        ...
        const content = readFileSync(
          path.join(tmpDir, "nestedMaxOccurs.ts"),
          "utf8"
        );
    ...
    test("combines element maxOccurs with compositor maxOccurs", () => {
      withTmpDir((tmpDir) => {
        ...
        const content = readFileSync(
          path.join(tmpDir, "combinedMaxOccurs.ts"),
          "utf8"
        );
    ...
    test("XMLSchema.xsd schema element has array properties", () => {
      withTmpDir((tmpDir) => {
        ...
        expect(schemaContent).toMatch(
          /@XmlElement\('element',\s*\{[^}]*array:\s*true[^}]*\}/
        );
        ...
        expect(schemaContent).toMatch(
          /complexType\?:\s*topLevelComplexType\[\]/
        );
        expect(schemaContent).toMatch(/group\?:\s*namedGroup\[\]/);
        expect(schemaContent).toMatch(
          /attributeGroup\?:\s*namedAttributeGroup\[\]/
        );
      });
    });
  });
  ...
  describe("minOccurs on compositors", () => {
    test("elements in compositors with minOccurs=0 are optional", () => {
      withTmpDir((tmpDir) => {
        ...
        const content = readFileSync(
          path.join(tmpDir, "optionalCompositors.ts"),
          "utf8"
        );
    ...
    test("nested optional compositors propagate optionality", () => {
      withTmpDir((tmpDir) => {
        ...
        const content = readFileSync(
          path.join(tmpDir, "nestedOptional.ts"),
          "utf8"
        );
  ...
  describe("minOccurs and maxOccurs on elements", () => {
    test("elements with minOccurs=0 are optional", () => {
      withTmpDir((tmpDir) => {
        ...
        const content = readFileSync(
          path.join(tmpDir, "elementMinOccurs.ts"),
          "utf8"
        );
    ...
    test("elements with minOccurs > 1 are arrays", () => {
      withTmpDir((tmpDir) => {
        ...
        const content = readFileSync(
          path.join(tmpDir, "minOccursGreaterThanOne.ts"),
          "utf8"
        );
